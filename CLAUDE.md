...
- **Traefik v2.11** — Reverse proxy. Routes `<REDACTED_N8N_HOST>` to the n8n container (port 5678). Config split between static (`traefik/traefik.yml`) and dynamic (`traefik/dynamic/config.yml`) with hot-reload. Docker provider discovers services via labels; `exposedByDefault: false` requires explicit opt-in.
...
- Production domain: `<REDACTED_N8N_HOST>`
